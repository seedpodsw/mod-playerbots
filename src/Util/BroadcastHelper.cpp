...
bool BroadcastHelper::BroadcastGuildGroupOrRaidInvite(PlayerbotAI* ai, Player* /* bot */, Player* player, Group* group)
{
    ...
    if (group && group->isRaidGroup())
    {
        if (urand(0, 3))
        {
            return ai->SayToGuild(PlayerbotTextMgr::instance().GetBotTextOrDefault(
                "broadcast_guild_raid_invite_generic", "Hey, anyone want to raid in %zone_name?", placeholders));
        }
        else
        {
            return ai->SayToGuild(PlayerbotTextMgr::instance().GetBotTextOrDefault(
                "broadcast_guild_raid_invite_named", "Hey %name, I'm raiding in %zone_name, do you want to join me?",
                placeholders));
        }
    }
    else
    {
        ...
        {
            return ai->SayToGuild(PlayerbotTextMgr::instance().GetBotTextOrDefault(
                "broadcast_guild_group_invite_generic", "Hey, anyone wanna group up in %zone_name?", placeholders));
        }
        else
        {
            return ai->SayToGuild(PlayerbotTextMgr::instance().GetBotTextOrDefault(
                "broadcast_guild_group_invite_named",
                "Hey %name, do you want to join my group? I'm heading for %zone_name.", placeholders));
        }
    }
}
